...
% sim('forward_ideal.slx',[0,0.3],options);
...
hold off
grid on
title('Input and Magnetizing Current')
legend('input current','magnetizing current')
xlabel('time [sec]','FontSize',18)
...
% legend('MOSFET Voltage');
% xlabel('time [sec]','FontSize',18)
% ylabel('Voltage [V], Amperes [A]','FontSize',18)
% xlim([0.06,0.0602]);
% xticks([0.06, 0.0601, 0.0602])
% set(findall(gcf,'Type','line'),'LineWidth',2)
% set(gca,'FontSize',12)
% ylim([-1,150])
% 
